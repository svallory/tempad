---
name: tempad-quest
description: Declare which quest this session (or subagent) is pursuing, so tempad tracks work under the right objective instead of guessing. Use at session start, when the objective changes, and inside every subagent before it starts work.
---

# tempad-quest

TemPad no longer guesses what you're working on — you declare it. Declare early and re-declare
whenever the objective shifts.
...
## Re-declare on objective change

When the prompt shifts what you're doing mid-session:

- If the old objective is simply done, declare the new one plainly (no `--origin`).
...
```
tempad quest declare --session <id> --new "<title>" --objective "<text>" \
...
when its objective is the same as the parent's:

```
tempad quest declare --session <subagent session id> --parent <parent session id> \
  --quest <quest id> --plan "step one; step two"
```
...
from "same objective on purpose."
