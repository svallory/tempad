...
case "$session_id" in
  "" ) exit 0 ;;
  *[!A-Za-z0-9_-]* )
    echo "$(date -u +%Y-%m-%dT%H:%M:%S.000Z) w5-prompt.sh: rejecting session id with unexpected characters" >> "$log_file"
    exit 0
    ;;
esac
...
_bin=($TEMPAD_BIN)
context="$("${_bin[@]}" w5 context --session "$session_id" 2>/dev/null || true)"
