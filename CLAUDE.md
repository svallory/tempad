...
**Classifier memory slice.** `buildWindow` (`src/w5/window.ts`) hands the classifier a recent-context slice instead of a single previous trace: `sessionOpenActivities` (this session's still-open activities), `recentActivities` (activities in the same `org/project` opened or closed within `memory_hours`, capped at `memory_activities`), `recentSideQuests` (the last 3 branched quests with their trigger), `overlapMessages` (the tail before the window's cut, context only), and `previousSessionNote` (`w5_runs.session_note`). `recentActivities` includes _closed_ activities of the current session as well as other sessions', because an idle gap mid-session closes an activity and returning to it afterwards is exactly a `continues` link; still-open ones live only in `sessionOpenActivities`, so nothing appears twice. A whole-session window (`sinceTs` null, as backfill builds) has no overlap tail — there is nothing before its first message.

Each `ClassifierSegment` must set exactly one of `matchedActivity` (reuse an open activity), `continuesActivity` (resume a closed one) or `newActivityReason` (why neither fits); `validateResult` rejects zero or two. `ClassifierResult` also carries a top-level `sessionNote` (at most 300 characters), written to `w5_runs.session_note` by `completeJob`'s trailing `sessionNote` parameter and fed back on the session's next run. `applyResult` never reassigns a matched activity's quest: a disagreement with `matchedQuest` counts a `questConflict` in the summary and logs a line. Its summary also carries `overlapDropped`, segments discarded for falling entirely inside the overlap range.
