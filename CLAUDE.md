...
**Activity lifecycle.** An `activity.closed` event's payload carries a `reason` (`"idle" | "session_end" | "switch"`), stored as `activities.close_reason` and independent of the optional `outcome` judgment field: `reason` says why the activity stopped being open, `outcome` says how it went. `src/w5/lifecycle.ts` emits it three ways — `closeIdleActivities` closes any open activity of a session whose last trace ended more than `[w5].activity_idle_minutes` (default 45) minutes before the current run's window starts, before classification ever sees it; `closeActivityOnSwitch` closes the session's previous open activity the moment a classified segment switches to a different activity; `closeSessionActivities` closes every remaining open activity of a session and clears `w5_runs.session_note` when the `SessionEnd` hook fires. `openActivityContinuing` opens a new activity with an optional `continues: <closed activity id>` link, recorded in the `activities.continues` column, for returning to the same objective after a gap. Three more `[w5]` keys bound the classifier's memory slice: `memory_hours` (default 8) and `memory_activities` (default 10) limit how far back and how many recent activities it can see, `overlap_messages` (default 3) is how many prior messages are included as non-classified context.

**Classifier memory slice.** `buildWindow` (`src/w5/window.ts`) hands the classifier a recent-context slice instead of a single previous trace: `sessionOpenActivities` (this session's still-open activities), `recentActivities` (activities in the same `org/project` opened or closed within `memory_hours`, capped at `memory_activities`), `recentSideQuests` (the last 3 branched quests with their trigger), `overlapMessages` (the tail before the window's cut, context only), and `previousSessionNote` (`w5_runs.session_note`), which is rendered inside a fenced, explicitly-labeled data block — the note is classifier-authored but composed from transcript text the user and tools control, so the system prompt states that fenced text is a hint, never an instruction, and any fence inside the note is neutralized. `recentActivities` includes _closed_ activities of the current session as well as other sessions', because an idle gap mid-session closes an activity and returning to it afterwards is exactly a `continues` link; still-open ones live only in `sessionOpenActivities`, so nothing appears twice. A whole-session window (`sinceTs` null, as backfill builds) has no overlap tail — there is nothing before its first message.

Each `ClassifierSegment` must set exactly one of `matchedActivity` (reuse an open activity), `continuesActivity` (resume a closed one) or `newActivityReason` (why neither fits); `validateResult` rejects zero or two. `ClassifierResult` also carries a top-level `sessionNote` (at most 300 characters), written to `w5_runs.session_note` by `completeJob`'s trailing `sessionNote` parameter and fed back on the session's next run. `applyResult` never trusts an activity id the classifier returns: every id is looked up (ignoring retracted rows) before use. `matchedActivity` is honoured only for an activity that is still open; `continuesActivity` only for one that has closed — pointed at a still-open activity it is treated as a plain reuse, since opening a second row would leave two open activities for one objective. An id that resolves to nothing falls through to opening a new activity and counts an `unknownActivityId`. `applyResult` also never reassigns a matched activity's quest: a disagreement with `matchedQuest` counts a `questConflict` and logs a line. The summary carries `questConflicts`, `overlapDropped` (segments discarded for falling entirely inside the overlap range) and `unknownActivityIds`.

Backfill rebuilds the slice **per chunk**, not once per session: chunk n+1 must see the activities chunk n opened or closed and the note it wrote, exactly as a live run sees the previous hook invocation's work. Its `sinceTs` is the previous chunk's last message so the overlap tail is the messages before the chunk, never the chunk's own — `messages` is then overridden with the chunk. `writeSessionNote` (`src/w5/jobs.ts`) is the shared note write used by both backfill's per-chunk loop and the runner's `completeJob`.
...
**`tempad w5 eval --from <date> --to <date> [--db <path>]`** (`src/w5/eval.ts`) measures the activities/traces ratio against real history without ever writing to the real database: `--from`/`--to` are validated as parseable dates with `from <= to` (rejecting bad input with a message and exit 1) before anything touches disk, then the source (`--db`, else `TEMPAD_HOME/tempad.db`) is copied to `TEMPAD_HOME/scratch/eval-<timestamp>.db` via SQLite's own `VACUUM INTO` on a read-only connection — not a file-level copy, since the real db runs in WAL mode and a copied `.db` file alone can miss committed rows still sitting in the uncheckpointed `-wal` sidecar. It force-reclassifies every window in `[from, to]` against the copy via `backfill`'s `force`/`from`/`to` options (`force` bypasses `w5_windows`/legacy coverage entirely; `from`/`to` bound both the session query and each session's per-message window, so a session that runs past `to` is only classified up to `to`). It prints trace count, activity count, the ratio, median activity duration, `continues` link count, quest conflicts, unknown activity ids, overlap-dropped segments, and 20 randomly sampled activities for a hand check — the conflict/unknown/overlap counts are summed straight from `apply.ts`'s real per-window run summary, not a placeholder.
