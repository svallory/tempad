...
**Activity lifecycle.** An `activity.closed` event's payload carries a `reason` (`"idle" | "session_end" | "switch"`), stored as `activities.close_reason` and independent of the optional `outcome` judgment field: `reason` says why the activity stopped being open, `outcome` says how it went. `src/w5/lifecycle.ts` emits it three ways — `closeIdleActivities` closes any open activity of a session whose last trace ended more than `[w5].activity_idle_minutes` (default 45) minutes before the current run's window starts, before classification ever sees it; `closeActivityOnSwitch` closes the session's previous open activity the moment a classified segment switches to a different activity; `closeSessionActivities` closes every remaining open activity of a session and clears `w5_runs.session_note` when the `SessionEnd` hook fires. `openActivityContinuing` opens a new activity with an optional `continues: <closed activity id>` link, recorded in the `activities.continues` column, for returning to the same objective after a gap. Three more `[w5]` keys support the classifier's future memory slice: `memory_hours` (default 8) and `memory_activities` (default 10) bound how far back and how many cross-session activities it can see, `overlap_messages` (default 3) is how many prior messages are included as non-classified context.
