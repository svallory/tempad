...
- ~~marko-ui package names~~ **Resolved** (corrected after ROUND 1 review; the first pass of this spec misquoted the CLI and understated what the docs actually say). `https://marko-ui.saulo.tech/docs/installation` documents the install flow verbatim:
  ```
  bun create marko my-app
  cd my-app
  bun install

  bunx marko-ui init
  bunx marko-ui init --distribution copy --visual-style vega
  bunx marko-ui init --distribution import --visual-style vega

  bunx marko-ui add button
  bunx marko-ui add dialog select toast
  ```
  `add` takes component names, not a URL — there is no `add <url>` form; that was an invented/misremembered syntax in the spec's first pass and is dropped. `init` takes `--distribution` (`copy` or `import`) and a required `--visual-style` (e.g. `vega`, one of 8 layers). On `import`, the docs state plainly: **"`init` writes `components.json`, adds `@marko-ui/shadcn` as a dependency"** — `@marko-ui/shadcn` is the runtime component package for the import distribution, not merely a styles package; it "ships the same components as `mu-*` hook-class components plus all 8 visual style layers," imported per-component as e.g. `@marko-ui/shadcn/ui/button/button.marko`. The "copy" distribution instead vendors component source directly into the app via `bunx marko-ui add <component>` (no package dependency beyond the underlying `@zag-js/*` packages the copied component code imports). `web-scaffold` still picks copy vs. import and records the choice in that task's commit, but the package name itself (`@marko-ui/shadcn`) is no longer an open question.
